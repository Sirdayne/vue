...
export default {
  ...
  computed: {
    ...
    cultureSorts() {
      return this.fromVuex('sorts')
    },
    reproductions() {
      return this.fromVuex('reproductions')
    },
    cultureFieldZones() {
      return this.fromVuex('culturefieldzones')
    },
    ...
    filteredSeedlimits() {
      ...
      let seedlimits = this.seedlimits.filter(x => {
        let date = moment(x.date).format("YYYY") === moment(Date.now()).format("YYYY")
  ...
  created() {
    this.fetchEntities([
      'seedlimits', 'fields', 'cultures', 'sorts', 'reproductions', 'culturefieldzones'
    ]);
